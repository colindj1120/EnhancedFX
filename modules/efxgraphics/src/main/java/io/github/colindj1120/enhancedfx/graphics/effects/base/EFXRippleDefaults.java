...
/**
 * Defines the default constants used for configuring {@link EFXRippleEffect} instances.
 *
 * <p>This class encapsulates a set of static final constants that represent the default values for various properties of the ripple effect and its associated drop shadow. These defaults serve as the baseline
 * configuration for ripple effects applied to UI components, ensuring a consistent look and feel across different implementations.</p>
 *
 * <p>The constants in this class include settings for ripple state, shape, color, duration, interpolator type, background presence, radius, stroke width, stroke color, direction, fade state, as well as drop
 * shadow properties like the blur type, color, radius, spread, and offset. These settings can be used directly or overridden by custom configurations in the ripple effect factory.</p>
 *
 * <p>This class is instrumental in maintaining a standard for visual effects, making it easier to apply uniform styles and behaviors for ripple effects throughout an application.</p>
 *
 * <h2>Usage example:</h2>
 * <pre>
 * {@code
