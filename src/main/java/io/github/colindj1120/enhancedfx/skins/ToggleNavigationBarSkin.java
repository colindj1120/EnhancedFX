...
import io.github.colindj1120.enhancedfx.collections.ObservableLinkedList;
import io.github.colindj1120.enhancedfx.controls.EnhancedToggleButton;
import io.github.colindj1120.enhancedfx.controls.ToggleNavigationBar;
...
/**
 * A custom skin for the {@link ToggleNavigationBar} control in the EnhancedFX framework, designed to dynamically adjust the layout and presentation of a navigation bar comprising toggle
 ...
 *
 * <p>The {@code ToggleNavigationBarSkin} extends {@link SkinBase} and integrates closely with {@link ToggleNavigationBar}, providing a flexible and customizable appearance for the navigation bar. It
 * manages a collection of {@link EnhancedToggleButton} instances, arranging them according to the specified orientation of the navigation bar and responding to user interactions.</p>
 *
 ...
 *   containers based on the orientation property of the {@link ToggleNavigationBar}.</li>
 ...
 * This skin also registers a {@link ListChangeListener} to the {@link ObservableLinkedList} of {@link EnhancedToggleButton},
 * handling the dynamic addition and removal of buttons from the navigation bar.
 *
 * <p>Action listeners are attached to the {@link ToggleNavigationBar}'s properties and the list of toggle buttons to ensure that any changes (such as orientation change or button list
 * modifications) are reflected in the UI without requiring manual updates.</p>
 *
 * <p><em>Usage:</em></p>
 * This skin is automatically applied to the {@link ToggleNavigationBar} upon its instantiation and requires no additional setup from the developer. However, customization can be achieved through
 * CSS and by manipulating the {@link ToggleNavigationBar}'s properties and toggle button list programmatically.
 *
 * <p><em>Example:</em></p>
 * <pre>
 * ToggleNavigationBar navBar = new ToggleNavigationBar();
 ...
 * @see SkinBase
 */
public class ToggleNavigationBarSkin extends SkinBase<ToggleNavigationBar> {
    ...
    /**
     * Constructs a new skin for the {@link ToggleNavigationBar}. This skin implementation manages the layout and styling of the {@link ToggleNavigationBar} control, dynamically adjusting its
     ...
     *     <li>Creates a binding for the current container, determining whether to use a horizontal or vertical container based on the {@code isHorizontal} property of the {@link ToggleNavigationBar}
     ...
     *     accordingly.</li>
     *     <li>Requests a layout update for the {@link ToggleNavigationBar} control, ensuring that its appearance is refreshed to reflect any initial settings or changes.</li>
     * </ul>
     * </p>
     *
     * @param control
     *         the {@link ToggleNavigationBar} control this skin is associated with. This control provides the necessary context and properties that influence the skin's behavior and appearance.
     */
    public ToggleNavigationBarSkin(ToggleNavigationBar control) {
        super(control);
    ...
    /**
     * Registers an action listener to an {@link ObservableLinkedList} of {@link EnhancedToggleButton}. This listener responds to changes in the list, such as additions, removals, and replacements of
     ...
     */
    private void addActionListener(ObservableLinkedList<EnhancedToggleButton> buttonList) {
        buttonList.addActionListener((listChangeItem) -> {
            switch (listChangeItem.getListAction()) {
                case ADDED -> addToggleButton(listChangeItem.getNewElement());
                case BULK_ADD -> addBulkToggleButtons(listChangeItem.getNewList());
                case REMOVED -> removeToggleButton(listChangeItem.getOldElement());
                case BULK_REMOVE -> removeBulkToggleButtons(listChangeItem.getOldList(), listChangeItem.getNewList());
                case REPLACED -> replaceToggleButton(listChangeItem.getOldElement(), listChangeItem.getNewElement());
                case BULK_REPLACED -> replaceBulkToggleButtons(listChangeItem.getOldList(), listChangeItem.getNewList());
                case CLEARED -> clearContainer();
            }
        });
    }
     ...
     */
    private void replaceBulkToggleButtons(List<EnhancedToggleButton> oldList, List<EnhancedToggleButton> newList) {
     ...
     */
    private void replaceToggleButton(EnhancedToggleButton oldElement, EnhancedToggleButton newElement) {
    ...
    /**
     * Adds a list of toggle buttons to the UI container in bulk. This method filters the list to include only instances of {@link EnhancedToggleButton} and then adds each button to the container.
     *
     * @param buttonList
     ...
     *
     * @param toggleButton
     *         the {@link EnhancedToggleButton} to be added to the container.
     */
    private void addToggleButton(EnhancedToggleButton toggleButton) {
        getCurrentContainer().getChildren()
                             .add(toggleButton);
    }
     ...
     *
     * @param toggleButton
     *         the {@link EnhancedToggleButton} to be removed from the container.
     */
    private void removeToggleButton(EnhancedToggleButton toggleButton) {
        getCurrentContainer().getChildren()
                             .remove(toggleButton);
    }
     ...
     */
    private void removeBulkToggleButtons(List<EnhancedToggleButton> oldList, List<EnhancedToggleButton> newList) {
        List<EnhancedToggleButton> removedToggleButtons = getRemovedToggleButtons(oldList, newList);
     ...
     *
     * @return a list of {@link EnhancedToggleButton} that have been removed.
     */
    private List<EnhancedToggleButton> getRemovedToggleButtons(List<EnhancedToggleButton> oldList, List<EnhancedToggleButton> newList) {
