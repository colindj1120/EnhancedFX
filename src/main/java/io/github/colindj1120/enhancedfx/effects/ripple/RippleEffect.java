 ...
 */
package io.github.colindj1120.enhancedfx.effects.ripple;

import io.github.colindj1120.enhancedfx.animation.AnimationManager;
import io.github.colindj1120.enhancedfx.converters.InterpolatorStyleConverter;
...
import io.github.colindj1120.enhancedfx.shapes.AsymmetricRoundedRectangle;
...
import static io.github.colindj1120.enhancedfx.utils.AnimationUtils.*;
 ...
 * shapes determined by {@link RippleShape}
 *
...
public class RippleEffect extends Region {
    ...
    static {
        stylesManager.addCssMetaData(CssFactory.<RippleEffect, State>create()
    ...
    /**
     * Constructs a {@link Shape} to be used as a clipping mask based on the current {@code rippleShape} configuration. This method determines the appropriate clipping shape to match the ripple effect shape,
     ...
     * {@code rippleShape}. Depending on the configured {@code rippleShape} value, it delegates to specific methods to create the appropriate {@link Shape} instance for the ripple effect, such as a rectangle,
     ...
     *
     * <p>
     * This implementation ensures that a {@link RippleEffectException} is thrown if {@code rippleShape} is not set, enforcing the requirement that a valid shape must be specified to create a ripple effect.
    ...
    private void initializeStyleableProperties() {
        ...
        rippleShape = ExtendedStyleableObjectPropertyFactory.<RippleShape>builder()
                                                            .name("rippleShape")
                                                            .bean(RippleEffect.this)
                                                            .type(RippleShape.class)
                                                            .cssMetaData(stylesManager.findCssMetaData("-efx-ripple-shape"))
                                                            .defaultValue(DEFAULT_RIPPLE_SHAPE)
                                                            .build();

        rippleClipShape = ExtendedStyleableObjectPropertyFactory.<RippleShape>builder()
        ...
        rippleDirection = ExtendedStyleableObjectPropertyFactory.<RippleDirection>builder()
                                                                .name("rippleDirection")
                                                                .bean(RippleEffect.this)
                                                                .type(RippleDirection.class)
